...
export class ListPdfPage implements OnInit {
  ...
  async downloadForm(data) {
    ...
    try {
      ...
      if (parts.propSol === "PS-Facilities-Site") {ps_facilities.check()}
      if (parts.propSol === "PS-Projects") {ps_projects.check()}
      ...
      if (taskobs.ass_high_risk === "Working at Height" ) { work_at_height.check() }
      if (taskobs.ass_high_risk === "Working Near Moving Equipment" ) { work_near_move.check()}
      if (taskobs.ass_high_risk === "Lifting and Rigging" ) { lift_rigg.check()}
      if (taskobs.ass_high_risk === "Excavation" ) {excavation.check()}
      if (taskobs.ass_high_risk === "Energy Isolation" ) {ener_iso.check()}
      if (taskobs.ass_high_risk === "Hot Work" ) {host_work.check()}
      if (taskobs.ass_high_risk === "Confined Space" ) {conf_space.check()}
      if (taskobs.ass_high_risk === "Defeat of Critical Safety Devices" ) {defeat_of_crit.check()}
      if (taskobs.ass_high_risk === "Does not apply" ) {does_not_apply.check()}
      ...
      session_time.setText(moment(taskobs.fb_sess_time).format("HH:mm") + pm_am)
